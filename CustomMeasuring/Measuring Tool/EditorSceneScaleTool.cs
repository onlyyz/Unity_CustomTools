...
public class EditorSceneScaleTool : MonoBehaviour
{
    ...
    private void Line(Vector3 v1, Vector3 v2, Vector3 start)
    {
        ...
        else if (sizeType == DistanceType.LineCoord) tempDis = scaleSize * 1.0f;
    ...
    private void LineScaleLabel(Vector3 v1, Vector3 v2, Vector3 start)
    {
        ...
        else if (sizeType == DistanceType.Coordinates) Coordinates(v1, v2, start);
        else if (sizeType == DistanceType.LineCoord) LineCoord(v1, v2, start);
    }
    ...
    private void Coordinates(Vector3 v1, Vector3 v2, Vector3 start)
    {
        int coord = 0;
        for (float i = 0; coord <= scaleSize; i += (ydToMeter * unit))
        {
            Label(start + v1 * i, coord);
            Label(start + v2 * i, -coord);
            coord+=unit;
        }
    }
    private void LineCoord(Vector3 v1, Vector3 v2, Vector3 start)
    {
        int coord = 0;
        for (float i = 0; coord <= scaleSize; i += (ydToMeter * unit))
        {
